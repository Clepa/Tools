# Pending

Debug: [VSCode JS debug](https://github.com/microsoft/vscode-js-debug).

## Other links

[extension-pack-roundup](https://code.visualstudio.com/blogs/2017/03/07/extension-pack-roundup)
[awesome-vscode](https://viatsko.github.io/awesome-vscode/#camel-humps)
[how-can-you-export-the-visual-studio-code-extension-list](https://stackoverflow.com/questions/35773299/how-can-you-export-the-visual-studio-code-extension-list)
[flutter-visual-studio-code-shortcuts-for-fast-and-efficient-development-7235bc6c3b7d](https://medium.com/flutter-community/flutter-visual-studio-code-shortcuts-for-fast-and-efficient-development-7235bc6c3b7d)
[how-to-export-vscode-settings](https://stackoverflow.com/questions/35368889/how-to-export-vscode-settings)
[extension-gallery](https://code.visualstudio.com/docs/editor/extension-gallery)
[markdown-basics](https://shd101wyy.github.io/markdown-preview-enhanced/#/markdown-basics?id=what-is-markdown)
[vertical-rulers-in-visual-studio-code](https://stackoverflow.com/questions/29968499/vertical-rulers-in-visual-studio-code?rq=1)

## Extensions

[bierner.docs-view](https://marketplace.visualstudio.com/items?itemName=bierner.docs-view)
[ms-vscode.vscode-js-profile-flame](https://marketplace.visualstudio.com/items?itemName=ms-vscode.vscode-js-profile-flame)
[meganrogge.template-string-converter](https://marketplace.visualstudio.com/items?itemName=meganrogge.template-string-converter)
[kisstkondoros.vscode-codemetrics](https://marketplace.visualstudio.com/items?itemName=kisstkondoros.vscode-codemetrics)

### Create extensions

[your-first-extension](https://code.visualstudio.com/api/get-started/your-first-extension)
[vscode-api](https://code.visualstudio.com/api/references/vscode-api)

## JAVA

[java-tutorial](https://code.visualstudio.com/docs/java/java-tutorial)
[java](https://code.visualstudio.com/docs/languages/java)

[vscjava.vscode-java-pack](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-pack)
[vscjava.vscode-java-debug](https://marketplace.visualstudio.com/items?itemName=vscjava.vscode-java-debug)
[redhat.java](https://marketplace.visualstudio.com/items?itemName=redhat.java)
